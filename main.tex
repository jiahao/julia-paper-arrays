...
\begin{figure}
  \caption{\label{fig:langdesign}Design of dynamic languages and their implementations: traditionally, dynamic languages were created by language designers before considerations of program analyses, which were left as follow-up details for implementation. In contrast, the Julia language is designed from the beginning to facilitate program analyses, with a single community responsible for both language design and implementation.}
  \centering
...
As shown in Figure~\ref{fig:dispatch}, multiple dispatch (also known as generic functions, or multi-methods) is an
...
\begin{figure}
  \caption{\label{fig:dispatch}Object-oriented method dispatch vs. multiple dispatch. Julia employs multiple dispatch to implement a wide variety of semantics.}
  \centering
