...
Array libraries typically handle dimensions recursively, one at a time; for arrays with ranks are known at compile time allow the compiler to infer the amount of storage needed for the shape information, and potentially unroll index computations fully.

%The dispatch semantics also allows for efficient
...
Most dynamic languages are designed without consideration for program
analysis, leaving this to others---future compiler writers and researchers---who
step in if the language becomes popular enough to warrant it.
Julia is designed with analysis in mind, with a single community responsible 
...
In this section, we describe how multiple dispatch, as a core feature of the Julia language\cite{Bezanson:2012jf}, is used to implement the \code{Array}
data type, which is parameterized by an element type and a rank (an integer).\footnote{For purposes of this paper, an \code{Array} can be considered a tuple of
...
The recursive approach to array semantics that is favored in static languages often encourages a design decision that lends itself naturally to the semantic structure of the language. For example, an indexing expression
of a 3-array in C++ might be written as \code{A[i][j][k]}. Here there are 
three applications of \code{operator[]}, each of which will decide whether or not to
...
dimensions on top of the dimensions indexing into individual color planes. In
...
means that the result is most conveniently expressed wirh rank $(n-k)$
rather than an $n$-array with $k$ singleton dimensions.

%In linear algebra on the other hand, one might want a row or column of a
