...
long history of special-purpose compiler optimizations to make operations
over arrays efficient \cite{Randell:1964a6, Busam:1969oe}. Many
implementations of programming languages therefore choose to build array
semantics into compilers.

Of the languages that offer $n$-arrays, only a few have sufficient power to
express the semantics of $n$-arrays for general $n$ in the same language,
i.e. without resorting to hard coding array behavior into a compiler written
in a different language. Library writers can in fact implement array data
types in such languages with sufficient expressive power. Notable examples 
include the C++ libraries \code{Blitz++} \cite{Veldhuizen:1998ab} or
\code{Boost.MultiArray} \cite{Garcia:2005ma}, and the Haskell Repa (Regular
Parallel Arrays) library \cite{Keller:2010rs,Lippmeier:2011ep,
Lippmeier:2012gp}.

These libraries are powerful. You can do general rank $n$ and through the magic of static analysis and induction, the recursive definition of $n$-arrays as the outer product
of a 1-array with an $(n-1)$-array leads naturally to a general implementation of $n$-arrays \cite{Bavestrelli:2000ct}. The dispatch semantics also allow for some neat tricks. For example, Repa allows for dispatch on the index-to-offset mapping function \cite{Keller:2010rs}, allowing for efficient matrix transpositions simply by changing the
\code{Shape} class to redefine the memory striding rules \cite{Keller:2010rs}. Using static analysis means that you can have zero runtime overhead and thus have performance.

These libraries are powerful, but nevertheless still have some limitations.
First, the language semantics required to achieve the necessary flexibility of
expression and performance, such as C++ expression templates, are only
available at compile time. These libraries would not be able to support
programs using $n$-arrays where $n$ is known only at runtime. C++ template
metaprograms, in particular, are also notoriously difficult to implement and
comprehend. Second, implementations of $n$-arrays tend to employ a recursive
peeling semantic where $n$-arrays are essentially defined as the outer product
of a 1-array with an $(n-1)$-array, for example the recursive template
implementation of \cite{Bavestrelli:2000ct}. The recursive peeling design also
restricts the naturability of indexing semantics to traverse indices in only
direction, e.g. from leftmost to rightmost. For example, \code{Repa}'s
reductions like \code{Sum} are only defined naturally over the last index
\cite{Keller:2010rs}; traversals over other indices involve some permutations
of the indices which require more code to express, and are furthermore not
guaranteed to produce an optimal memory traversal pattern.  As a consequence
of the recursive peeling design, to improve performance, production quality
libraries contain implementations that tend to avoid explicit recurrences, but
this necessitates a great many special-cased implementations of $n$-arrays to
ensure that the compiler is able to fully exploit its static analyses such as
loop unrolling to achieve maximal performance \cite{Garcia:2005ma,
Lippmeier:2011ep}. This also introduces a lot of repetition in the code
\cite{Lippmeier:2012gp}.

These libraries use static analysis and static semantics extensively. There are some applications, however, call for semantics that are not amenable to
...
and accept that most or all operations will be dynamically dispatched.However, languages in this category are not without their pitfalls. First,
