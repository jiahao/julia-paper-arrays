...
Arguably, arrays were the earliest data structure to be conceived of, and implemented on, computers. Implementation specifications for three-dimensional arrays go back to Konrad Zuse's Plankalkül, and were also described in the programmer's manual of FORTRAN I. The programmers manual describes that:

%TODO cite http://www.catb.org/retro/plankalkuel/

The elements of the array are stored sequentailly in memory.
The elements are in column-major order
The elements are stored from bottom right to top left.
