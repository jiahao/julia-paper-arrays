...
Array libraries built using compile-time abstraction, despite their good
performance, nevertheless have some limitations.
...
don't want to drop or rearrange dimensions too often. In linear algebra on
the other hand,
one might want a row or column of a matrix to become a vector by dropping
a dimension.
...
is a new compromise that we find compelling.
...
two where there were none before).
...
and so even dynamic languages that lack static analyses eventually want them
one way or another.

We believe that the combination of dataflow type inference and multiple
